...
class BackupManager:
    ...
    async def create_backup(
        ...
        async with async_session() as session:
            ...
            await self._enforce_max_backups(server_id)
    ...
    async def restore_backup(self, backup_id: int) -> dict:
        ...
        async with async_session() as session:
            ...
            if server_manager.is_running(server.id):
                return {
                    "ok": False,
                    "error": "Server must be stopped before restoring a backup",
                }
    ...
    async def _enforce_max_backups(self, server_id: int):
        async with async_session() as session:
            server = await session.get(Server, server_id)
    ...
    async def _enforce_retention(self, server_id: int):
        async with async_session() as session:
            result = await session.execute(
                select(Backup)
                .where(Backup.server_id == server_id)
